...
6. **Environment-Identifying Output & Schema Grounding Verification:** Any full-suite or environment-identifying output (such as Python version, pytest version, test file counts, OS platform, or file listings) must be checked against what earlier phase logs already established as the real environment before being reported. A mismatch is itself a signal to stop, investigate, and re-run against the real environment — never report mismatched or fabricated headers. Furthermore, before writing any claim about a table name, column name, file name, or endpoint path in a report or doc, the actual confirming command (e.g. database schema query, directory listing, or grep) must have been run in that exact same session. Never rely on remembered schemas or assumed naming conventions. This rule specifically guards against incidents such as `audit_log`, `work_order_evidence`, and `nagar_pragati_ward_snapshot` appearing in a performance report when none of them exist on the live database.
