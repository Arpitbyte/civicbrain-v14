...
6. **Environment-Identifying Output Consistency Verification:** Any full-suite or environment-identifying output (such as Python version, pytest version, test file counts, OS platform, or file listings) must be checked against what earlier phase logs already established as the real environment before being reported. A mismatch is itself a signal to stop, investigate, and re-run against the real environment — never report mismatched or fabricated headers.
