...
5. **RLS is the real authorization boundary:** Row-Level Security in PostgreSQL is the true perimeter, tested via the client SDK using real per-role JWTs, never tested via the SQL editor or a service-role bypass connection.
